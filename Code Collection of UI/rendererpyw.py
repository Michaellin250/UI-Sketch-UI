...
import sys, os
import timeit
import visual_hull as vh
from torchvision.utils import save_image, make_grid
import sketch as sk # for generate sketch from the shape


cuda = True if torch.cuda.is_available() else False   #make them global for other scripts to call functions in this script
Tensor = torch.cuda.FloatTensor if cuda else torch.FloatTensor
grid_global = Tensor([0,0,5])
...
def generate_image(bounding_box_min_x, bounding_box_min_y, bounding_box_min_z, \
    ...
    voxel_size, grid_res_x, grid_res_y, grid_res_z, width, height, grid, camera, back):   #don't need the camera_list

    # Get normal vectors for points on the grid, sure
    [grid_normal_x, grid_normal_y, grid_normal_z] = get_grid_normal(grid, voxel_size, grid_res_x, grid_res_y, grid_res_z)

    # Generate rays
    e = camera   #camera matrix??
    ...
    shading = 0
    l_dot_n = torch.sum(light_direction_point * intersection_normal, 2).unsqueeze_(2)
...
def initial_setting_basicshape(objname):
    ...
    grid_res_x = grid_res_y = grid_res_z = 64
    voxel_size = Tensor([4. / (grid_res_x-1)])

    # load sdf file
    #grid = read_sdf("SDF Objects/bunny.sdf", grid_res_x, bounding_box_min_x, bounding_box_max_x, 4. / (grid_res_x-1))
    if objname =="cube":
        grid = grid_construction_cube(64,-2,2)
    elif objname =="cylinder":
        grid = grid_construction_rounded_cylinder(64,-2,2,0,1,1)
    ...
    for cam in range(len(camera_list)):             
        ...
        bounding_box_max_x, bounding_box_max_y, bounding_box_max_z, \
        4. / (grid_res_x-1), grid_res_x, grid_res_y, grid_res_z, width, height, grid, camera_list[cam]+ torch.randn_like(camera_list[0]) * 0.015, 1)

        torchvision.utils.save_image(image, "./" + dir_name + objname+ ".png", nrow=8, padding=2, normalize=False, range=None, scale_each=False, pad_value=0)
    
    objfilepath = "results/"+objname+ ".png"
    return grid,objfilepath

#Multi Input
def initial_setting_filepath(filePath):  # adding a path toward multiple objects like bunny, cube later
    ...
    # load sdf file 
    grid = read_sdf(filePath, grid_res_x, bounding_box_min_x, bounding_box_max_x, 4. / (grid_res_x-1))
    #grid = torch.load("SDF Objects/initial.pt")
    #print(grid_global.shape)
    objname = "opened"
...
def initial_setting():  # adding a path toward multiple objects like bunny, cube later
    ...
    # load sdf file 
    grid_global = grid = read_sdf("SDF Objects/bunny.sdf", grid_res_x, bounding_box_min_x, bounding_box_max_x, 4. / (grid_res_x-1))
    objname = "bunny"
    ...
    objfilepath = "results/"+dir_name + objname+ ".png"
    return grid_global

def update_image(newpose, grid_new, objname):
    dir_name = "results/"
    os.makedirs("./" + dir_name, exist_ok=True)
    ...
    image = generate_image(bounding_box_min_x, bounding_box_min_y, bounding_box_min_z, \
        bounding_box_max_x, bounding_box_max_y, bounding_box_max_z, \
        4. / (grid_res_x-1), grid_res_x, grid_res_y, grid_res_z, width, height, grid_new, camm+ torch.randn_like(camm) * 0.015, 1)

    torchvision.utils.save_image(image, "./" + dir_name + objname+ ".png", nrow=8, padding=2, normalize=False, range=None, scale_each=False, pad_value=0)


#save as images for the masks
def update_mask(newpose, grid_new, objname):
    dir_name = "results/"
    os.makedirs("./" + dir_name, exist_ok=True)
    # check cuda
    cuda = True if torch.cuda.is_available() else False
    ...
    grid_res_x = grid_res_y = grid_res_z = 64   #preset parameters

    mask_target = vh.generate_mask(bounding_box_min_x, bounding_box_min_y, bounding_box_min_z, \
            bounding_box_max_x, bounding_box_max_y, bounding_box_max_z, \
            4. / (grid_res_x-1), grid_res_x, grid_res_y, grid_res_z, width, height, grid_new, camm, 1)
            
    torchvision.utils.save_image(mask_target, "./" + dir_name + objname+".png", nrow=8, padding=2, normalize=False, range=None, scale_each=False, pad_value=0)
...
# save as images
def update_sketch(newpose, grid_new, objname): #sketch name
    dir_name = "results/"
    os.makedirs("./" + dir_name, exist_ok=True)
    ...
    depth_target, curvature_target, dir_derivatives_target, sketch_target = sk.generate_image_sketch(bounding_box_min_x, bounding_box_min_y, bounding_box_min_z, \
            bounding_box_max_x, bounding_box_max_y, bounding_box_max_z, \
            4. / (grid_res_x-1), grid_res_x, grid_res_y, grid_res_z, width, height, grid_new, camm, 1, curvature_threshold)

    torchvision.utils.save_image(sketch_target, "./" + dir_name + objname+".png", nrow=8, padding=2, normalize=False, range=None, scale_each=False, pad_value=0)
